/******/ (() => {
  // webpackBootstrap
  /******/ "use strict"; // ./scripts/leetcode/util.js
  ...
  function delay(func, wait, ...args) {
    return new Promise((resolve) =>
      setTimeout(() => resolve(func(...args)), wait),
    );
  }
  ...
  LeetCodeV2.prototype.markUploaded = function () {
    setLeethubSyncButtonState("success");
  };
  LeetCodeV2.prototype.markUploadFailed = function () {
    setLeethubSyncButtonState("fail");
  };
  ...
  let leethubSyncStyleInjected = false;

  /* Injects the CSS driving the Sync-w/-LeetHub button's loading -> success/fail -> reset animation. */
  const injectLeethubSyncButtonStyle = () => {
    if (leethubSyncStyleInjected) return;
    leethubSyncStyleInjected = true;
    ...
    .leethub-sync-btn {
      position: relative;
      overflow: hidden;
      width: var(--leethub-btn-w, auto);
      transition: width .35s cubic-bezier(.65,-.15,.25,1.15),
                  padding .35s cubic-bezier(.65,-.15,.25,1.15),
                  background-color .25s ease;
    }
    .leethub-sync-icon, .leethub-sync-label { transition: opacity .15s ease; }
    .leethub-sync-wrap.loading .leethub-sync-btn,
    .leethub-sync-wrap.success .leethub-sync-btn,
    .leethub-sync-wrap.fail .leethub-sync-btn {
    ...
    .leethub-sync-wrap.loading .leethub-sync-icon,
    .leethub-sync-wrap.loading .leethub-sync-label,
    .leethub-sync-wrap.success .leethub-sync-icon,
    .leethub-sync-wrap.success .leethub-sync-label,
    .leethub-sync-wrap.fail .leethub-sync-icon,
    .leethub-sync-wrap.fail .leethub-sync-label {
      opacity: 0;
      pointer-events: none;
    }
    .leethub-sync-wrap.success .leethub-sync-btn { background-color: #78b13f !important; }
    .leethub-sync-wrap.fail .leethub-sync-btn { background-color: #d9534f !important; }
    ...
    .leethub-sync-loader svg { width: 100%; height: 100%; display: block; }
    .leethub-sync-track, .leethub-sync-arc { fill: none; stroke-width: 3; }
    .leethub-sync-track { stroke: rgba(255,255,255,.35); }
    .leethub-sync-arc { stroke: #fff; stroke-linecap: round; stroke-dasharray: 44 18; }
    .leethub-sync-wrap.loading .leethub-sync-loader {
      opacity: 1;
      animation: leethub-sync-spin .8s linear infinite;
    }
    ...
    .leethub-sync-check-path, .leethub-sync-fail-path {
      fill: none;
      stroke: #fff;
      stroke-width: 4;
    ...
    .leethub-sync-check-path { stroke-dasharray: 32; stroke-dashoffset: 32; }
    .leethub-sync-fail-path { stroke-dasharray: 40; stroke-dashoffset: 40; opacity: 0; }
    .leethub-sync-wrap.success .leethub-sync-check { opacity: 1; }
    .leethub-sync-wrap.success .leethub-sync-check-path { animation: leethub-sync-draw .3s ease .05s forwards; }
    .leethub-sync-wrap.fail .leethub-sync-check { opacity: 1; }
    .leethub-sync-wrap.fail .leethub-sync-fail-path { opacity: 1; animation: leethub-sync-draw .3s ease .05s forwards; }

    @keyframes leethub-sync-spin { to { transform: rotate(360deg); } }
    @keyframes leethub-sync-draw { to { stroke-dashoffset: 0; } }
  `;
    document.head.appendChild(style);
  };

  /* Small spinner shown while a sync is in flight. */
  const createSyncLoader = () => {
    const span = document.createElement("span");
    span.className = "leethub-sync-loader";
    span.setAttribute("aria-hidden", "true");
    span.innerHTML =
      '<svg viewBox="0 0 28 28"><circle class="leethub-sync-track" cx="14" cy="14" r="10"></circle><circle class="leethub-sync-arc" cx="14" cy="14" r="10"></circle></svg>';
    return span;
  };

  /* Checkmark / X icon shown once a sync resolves; visibility toggled purely via CSS classes on the wrap. */
  const createSyncStatusIcon = () => {
    const svgNS = "http://www.w3.org/2000/svg";
    const svg = document.createElementNS(svgNS, "svg");
    svg.setAttribute("class", "leethub-sync-check");
    svg.setAttribute("viewBox", "0 0 24 24");
    svg.setAttribute("aria-hidden", "true");

    const checkPath = document.createElementNS(svgNS, "path");
    checkPath.setAttribute("class", "leethub-sync-check-path");
    checkPath.setAttribute("d", "M4 12.5 L9.5 18 L20 6");

    const failPath = document.createElementNS(svgNS, "path");
    failPath.setAttribute("class", "leethub-sync-fail-path");
    failPath.setAttribute("d", "M5 5 L19 19 M19 5 L5 19");
  ...
  /* Drives the Sync-w/-LeetHub button through loading -> success/fail -> reset -> idle.
   Only acts while the button is actually mid-sync, so unrelated auto-submit uploads
   (which also call markUploaded/markUploadFailed) leave an idle button untouched. */
  function setLeethubSyncButtonState(state) {
    const wrap = document.getElementById("leethub-sync-wrap");
    if (!wrap || !wrap.classList.contains("loading")) return;

    wrap.classList.remove("loading");
    wrap.classList.add(state);

    const holdMs = state === "success" ? 1500 : 2000;
    setTimeout(() => {
      wrap.classList.remove(state);
      wrap.classList.add("reset");
      setTimeout(() => {
        wrap.classList.remove("reset");
        const btn = wrap.querySelector(".leethub-sync-btn");
        if (btn) btn.style.removeProperty("--leethub-btn-w");
      }, 380);
    }, holdMs);
  }
  ...
  function addManualSubmitBtn(eventHandler) {
    ...
    if (
      btns.innerText.includes("Solution") &&
      !btns.innerText.includes("LeetHub")
    ) {
      injectLeethubSyncButtonStyle();

      const wrap = document.createElement("span");
      wrap.className = "leethub-sync-wrap";
      wrap.id = "leethub-sync-wrap";
      ...
      btn.setAttribute(
        "class",
        "leethub-sync-btn group whitespace-nowrap focus:outline-none text-label-r bg-green-s dark:bg-dark-blue-s hover:bg-green-3 dark:hover:bg-dark-blue-3 flex items-center justify-center gap-2 rounded-lg px-3.5 py-1.5 text-sm font-medium",
      );

      const icon = createGitIcon();
      icon.classList.add("leethub-sync-icon");
      btn.appendChild(icon);

      const label = document.createElement("span");
      label.className = "leethub-sync-label";
      ...
      btn.addEventListener("click", (e) => {
        ...
        btn.style.setProperty("--leethub-btn-w", btn.offsetWidth + "px");
        void btn.offsetWidth; // force reflow so the starting width is registered before collapsing
  ...
  function setupManualSubmitBtn(submitBtnHandler) {
    ...
    const pageObserver = new MutationObserver((_, observer) => {
      ...
      if (
        window.location.href.match(/leetcode\.com\/(.*)\/submissions\/(\d+)/)
      ) {
  ...
  /* harmony default export */ const submitBtn = setupManualSubmitBtn; // ./scripts/leetcode/readmeTopics.js
  ...
  function appendProblemToReadme(topic, markdownFile, hook, problem) {
    ...
    const beforeSection = markdownFile.slice(
      0,
      markdownFile.indexOf(leetCodeSectionStart),
    );
  ...
  function sortTopicsInReadme(markdownFile) {
    ...
    if (leetCodeSection == null)
      throw new util_LeetHubError("LeetCodeTopicSectionNotFound");
    ...
    topics = topics.map((section) => {
      ...
      if (topicHeaderIndex < leetCodeSectionStartIndex) {
        ...
        if (endTopicString == null)
          throw new util_LeetHubError("EndOfTopicNotFound");
  ...
  function slugSet(topicTags) {
    const slugs = (topicTags || [])
      .map(
        (t) =>
          t?.slug ||
          (t?.name ? t.name.toLowerCase().replace(/[^a-z0-9]+/g, "-") : null),
      )
      .filter(Boolean);
  ...
  function classifyPattern(topicTags, titleSlug = "") {
    ...
    if (has("binary-tree", "tree")) {
      if (titleHas("preorder", "pre-order"))
        return "Tree Traversal - Pre Order";
      if (titleHas("postorder", "post-order"))
        return "Tree Traversal - Post-Order";
      if (titleHas("inorder", "in-order")) return "Tree Traversal - In Order";
      if (
        has("breadth-first-search") ||
        titleHas("level-order", "zigzag-level-order")
      )
        return "Tree Traversal - Level Order";
    }
    if (has("binary-search-tree")) return "BST / Ordered Set";
    if (has("ordered-set") && !has("array", "string"))
      return "BST / Ordered Set";
    ...
    if (has("dynamic-programming")) {
      ...
      if (titleHas("longest-increasing-subsequence"))
    ...
    if (has("sliding-window")) {
      if (titleHas("fixed", "size-k", "k-length"))
        return "Sliding Window - Fixed Size";
    ...
    if (titleHas("maximum-subarray", "maximum-sum-circular-subarray"))
    ...
    if (has("heap-priority-queue")) {
      ...
      if (titleHas("kth-", "top-k", "k-closest", "k-frequent"))
    ...
    if (has("segment-tree", "binary-indexed-tree"))
      return "Binary Indexed Tree / Segment Tree";
  ...
  function getPatternFolder(topicTags, titleSlug, solutionCode = "") {
    ...
    const usesBitwiseOperator =
      /\^=?|(?:[\w.)\]])\s*[&|]=?\s*(?:[\w.(\[]|\d)|(?:[\w.)\]])\s*(?:<<|>>)\s*(?:[\w.(\[]|\d)|~\s*(?:[\w.(\[]|\d)/.test(
        solutionCode,
      );
  ...
  async function getLeetCodeProfileStats() {
    ...
    if (!response.ok)
      throw new Error(
        `LeetCode profile stats request failed: ${response.status}`,
      );
    ...
    if (!calendar || !submitStats)
      throw new Error("LeetCode profile stats were unavailable");

    const countFor = (list, difficulty) =>
    ...
    const acceptanceRate =
      totalSubmitted > 0
        ? Math.round((solved / totalSubmitted) * 1000) / 10
        : 0;
    ...
    if (!submissionDays.has(day.toISOString().slice(0, 10)))
  ...
  const buildRepoReadme = (stats, githubUsername = leetCodeUsername) => {
    ...
    const nextMilestone =
      milestones.find((m) => solved < m) ?? milestones[milestones.length - 1];
    const progressPct = Math.min(
      100,
      Math.round((solved / nextMilestone) * 100),
    );
...
---
  ...
  const getSolutionCommitMessage = (leetCode, problemName) => {
    ...
    const problemNumber =
      question?.questionFrontendId || problemName.match(/^0*(\d+)/)?.[1];
  ...
  const upload = async (
    ...
    console.log(
      `Successfully committed ${getPath(problem, filename)} to github`,
    );
  ...
  const setPersistentStats = async (localStats) => {
    ...
    const { leethub_token: token, leethub_hook: hook } =
      await leetcode_api.storage.local.get(["leethub_token", "leethub_hook"]);
    ...
    try {
      ...
      if (e.message === "409") {
        ...
        const mergedStatsEncoded = encode(
          JSON.stringify({ leetcode: mergedStats }),
        );
  ...
  const updateReadmeWithDiscussionPost = async (
    ...
    let responseSHA;
    const { leethub_token, leethub_hook } =
      await leetcode_api.storage.local.get(["leethub_token", "leethub_hook"]);
    ...
    return getGitHubFile(leethub_token, leethub_hook, directory, filename)
      ...
      .then((existingContent) =>
        shouldPreprendDiscussionPosts
          ? encode(addition + existingContent)
          : encode(existingContent),
      )
      .then((newContent) =>
        upload(
          leethub_token,
  ...
  async function uploadGitWith409Retry(
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      try {
        ...
        await delay(
          () => undefined,
          WAIT_FOR_GITHUB_API_TO_NOT_THROW_409_MS * (attempt + 1),
        );
        try {
    ...
    try {
      ...
      if (stats == null) {
        stats = {
          ...
          acceptanceRate: readFallback(
            existingReadme,
            /Acceptance-([\d.]+)%25/,
            parseFloat,
          ),
          currentStreak: readFallback(
            existingReadme,
            /Current%20Streak-(\d+)%20Days/,
          ),
      ...
      try {
        ...
        if (!isSuccessfulSubmission) {
          ...
          if (iterations > 9) {
            ...
            throw new util_LeetHubError(
              "Could not find successful submission after 10 seconds.",
            );
          }
          return;
        }
        ...
        /* Upload README */
        const uploadReadMe = await leetcode_api.storage.local
          .get("stats")
          .then(({ stats }) => {
            const shaExists =
              stats?.shas?.[problemPath]?.[readmeFilename] !== undefined;
        ...
        syncRepoReadme().catch((err) => {
          console.warn(
            "Solution uploaded, but the repository README could not be refreshed.",
            err,
          );
        });
        ...
        if (!alreadyCompleted) {
          ...
          incrementStats(leetCode.difficulty, problemPath)
            ...
            .catch((err) => {
              console.warn(
                "Solution uploaded, but persistent statistics could not be refreshed.",
                err,
              );
            });
        }
  ...
  function wasSubmittedByKeyboard(event) {
    ...
    return (
      isEnterKey && ((isMacOS && event.metaKey) || (!isMacOS && event.ctrlKey))
    );
  }
  ...
  const submitBtnObserver = new MutationObserver(function (
    _mutations,
    observer,
  ) {
    ...
    if (v2SubmitBtn && textarea) {
      ...
      if (!!!v2SubmitBtn.onclick) {
        textarea.addEventListener("keydown", (e) =>
          v2SubmissionHandler(e, leetCode),
        );
